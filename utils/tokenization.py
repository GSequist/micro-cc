...
def token_cutter(messages: list[dict], tokenizer, max_tokens: int) -> list[dict]:
    ...
    for msg in reversed(messages):  # Most recent first
        ...
        # Check content for memory reminders (reminders are always string content)
        ...
        if (
            role == "user"
            and "<system-reminder>" in content_str
            ...
            and len(critical["user"]) < 3
            and "<system_reminder>" not in content
        ):
                ...
                if has_tool_result:
                    ...
                    for block in content:
                        if (
                            ...
                            # String content (standard tool results)
                            if (
