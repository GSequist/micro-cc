...
async def test():
    ...
    # python -m models.litellm

    # 1) Test message conversion
    print("=== _msgs_to_openai ===\n")
    converted = _msgs_to_openai(test_messages)
    for i, msg in enumerate(converted):
        ...
        if msg.get("tool_calls"):
            extras.append(f"tool_calls=[{', '.join(tc['function']['name'] for tc in msg['tool_calls'])}]")
        if msg.get("thinking_blocks"):
            extras.append(f"thinking_blocks={len(msg['thinking_blocks'])}")
        if msg.get("tool_call_id"):
            extras.append(f"tool_call_id={msg['tool_call_id']}")
        extra_str = f"  [{', '.join(extras)}]" if extras else ""

        print(f"  [{i:2d}] {role:>10}: {preview}{extra_str}")

    # 2) Test tool schema conversion
    print("\n=== _tools_to_openai ===\n")
    test_tools = [
        {"name": "bash_", "description": "Run shell commands", "input_schema": {"type": "object", "properties": {"command": {"type": "string"}}, "required": ["command"]}},
        {"type": "mcp_toolset", "mcp_server_name": "pubmed"},
    ]
    openai_tools = _tools_to_openai(test_tools)
    print(f"  Input:  {len(test_tools)} (1 local + 1 MCP)")
    print(f"  Output: {len(openai_tools)} (MCP filtered)")
    print(f"  Format: {json.dumps(openai_tools[0], indent=4)}")

    # 3) Live API call (only if env vars set)
    if os.getenv("LITELLM_API_KEY"):
        print("\n=== Live API call ===\n")
        response = await l_model_call(test_messages, thinking=True)
        if response:
            for block in response.content:
                print(f"  type={block.type}  preview={str(block.text or block.thinking or block.name)[:100]}")
        else:
            print("  Response: None (all retries failed)")
    else:
        print("\n=== Skipping API call (LITELLM_API_KEY not set) ===")
